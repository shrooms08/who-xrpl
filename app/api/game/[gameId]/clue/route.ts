...
export async function POST(
  ...
  const result = await mutateGame(admin, gameId, (state) => {
    if (state.phase !== "clue") throw new Error("wrong_phase");
    if (currentCluePlayer(state) !== auth.user.id) throw new Error("not_your_turn");
    return submitClue(state, auth.user.id, text); // throws on invalid clue
  });
